    ...
    def bvp(self, 
        ...
        if self.inter_method=="Noise":
            ...
            if self.interpolation_space == "noise":
                ...
                timing_generative = timeit.repeat(lambda: self.sample_data_images(ldm, 
                                                                                  data_curve, 
                ...
                out_file=f"{out_dir}/timing_data.txt"
                with open(out_file, "w") as f:
                    f.write(f"mu_interpolation: {mu_interpolation:.4f}\n")
                    f.write(f"std_interpolation: {std_interpolation:.4f}\n")
                    f.write(f"mu_generative: {mu_generative:.4f}\n")
                    f.write(f"std_generative: {std_generative:.4f}\n")
                    f.write(f"mu_frac: {mu_frac:.4f}\n")
                    f.write(f"std_frac: {std_frac:.4f}\n")
        ...
        timing_generative = timeit.repeat(lambda: self.sample_images(ldm, 
                                                                     noisy_curve, 
                                                                     cond1, 
                                                                     uncond_base, 
                                                                     cur_step, 
        ...
        out_file=f"{out_dir}/timing_noise.txt"
        with open(out_file, "w") as f:
            f.write(f"mu_interpolation: {mu_interpolation:.4f}\n")
            f.write(f"std_interpolation: {std_interpolation:.4f}\n")
            f.write(f"mu_generative: {mu_generative:.4f}\n")
