...
def run_banana(N:int=10, lam:float=1.0, max_iter:int=100, inter_method:str="linear", clip:bool=False,
    ...
    CM = cm.ContextManager(N=N, lam=lam, max_iter=max_iter, inter_method=inter_method, clip=clip,
                           mu = mu, nu = nu,
                           ckpt_path=ckpt_path)
    img1 = Image.open('sample_imgs/banana1.png').resize((768, 768))
    img2 = Image.open('sample_imgs/banana2.png').resize((768, 768))
    ...
    CM.interpolate_new(img1, img2,  prompt=prompt, n_prompt=n_prompt, ddim_steps=200, guide_scale=10,  out_dir='sample_results/banana')
